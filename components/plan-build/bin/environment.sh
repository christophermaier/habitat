...
# TODO (CM): use double leading underscores to differentiate between
# my implementation and the "old" implementation
...
#
# TODO (CM): add ways for the user to selectively export certain
# environment variables into the build environment, per discussion
# with Blake
...
# TODO (CM): verify that PATH is built up in pkg_deps order and
# behaves in the same way as this environment logic.
                ...
                if [ -n env["${var}"] ]; then
                    ...
                    case $(__env_var_type "${var}") in
                        ...
                        aggregate)
                            ...
                            ;;
                        # TODO (CM): what if there's another type?
                    esac
                else
...
# Hrmm... I don't think we want an actual function invocation to set
# these things UNLESS it is inside of the do_before callback, because we need to
# control the execution of it. The alternative is to declare things in
# top-level datastructures and then resolve them early.
set_buildtime_env() {
...
set_env(){
    ...
    local env_name=${3}
    build_line "OMG THE ENV IS ${env_name}"
    __fail_on_unrecognized_env "${env_name}"
    declare -n env="${env_name}"
    declare -n provenance="$(__provenance_for_environment ${env_name})"

    # TODO (CM): steal backtrace functions from BATS to ensure this
    # only runs inside of do_before?
...
# TODO (CM): consider having provenance be a *chain* for aggregate
# values; as we pull in new values, we also pull in and build up a
# provenance chain.

# TODO (CM): sep should be the optional 3rd value, and should be taken
# from our internal map / hints if possible
...
do_push_env() {
    ...
    # TODO (CM): ensure that pkg_ident is set better
...
# TODO (CM): search core_plans and Github for uses of George's
# environment stuff
...
# # Push an item onto a path if it is not already present.
# # TODO (CM): Push to front and then dedupe? i.e., pull the item to the
# # front if it's already there?
# push_to_path(){
#     local item=${1}
#     local path=${2}
#     local separator=${3:-:}
...
#         "${item}"${separator}*)
#             ;&  # Start of the path
#       *${separator}"${item}"${separator}*)   # Middle of the path
#             ;;         # already there
#      *)
#          if [ "" == "${path}" ]; then
#              path="${item}"
#          else
#              path="${item}${separator}${path}"
#          fi
#        ;;    # not there yet
#     esac
#     echo $path
...
do_setup_environment_wrapper() {
    ...
    # TODO (CM): Also need to deal with _build_environment function!

    do_setup_environment

    # TODO (CM): preserve initial environment and restore it when
    # build exits; otherwise environments could leak across builds
    ...
    build_line "ENVIRONMENT"
    for k in "${!__runtime_environment[@]}"; do
        build_line "$k = ${__runtime_environment[${k}]}"
    done
    build_line "ENVIRONMENT DONE"

    build_line "PROVENANCE"
    for k in "${!__runtime_environment_provenance[@]}"; do
        build_line "$k = ${__runtime_environment_provenance[${k}]}"
    done
    build_line "PROVENANCE DONE"

    build_line "BUILD ENVIRONMENT"
    for k in "${!__buildtime_environment[@]}"; do
        build_line "$k = ${__buildtime_environment[${k}]}"
    done
    build_line "BUILD ENVIRONMENT DONE"
    build_line "BUILD PROVENANCE"
    for k in "${!__buildtime_environment_provenance[@]}"; do
        build_line "$k = ${__buildtime_environment_provenance[${k}]}"
    done
    build_line "BUILD PROVENANCE DONE"
